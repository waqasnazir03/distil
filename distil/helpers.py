...
def get_max_last_collected(max_collection_start_age):
    """Work out the maximum ``last_collected`` value
    for the current time and the given max collection start age.

    :param max_collection_start_age: Max collection start age, in hours
    :type max_collection_start_age: int
    :return: Current maximum `last_collected` value
    :rtype: datetime.datetime
    """

    return (
        datetime.utcnow().replace(minute=0, second=0, microsecond=0)
        - timedelta(hours=max_collection_start_age)
    )
